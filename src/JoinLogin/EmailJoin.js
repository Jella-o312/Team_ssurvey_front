...
{/*😡😡😡이거 이상함 😡😡😡*/}
  const [checkPw, setCheckPw]= useState('');  // 비밀번호 중복 체크 저장 스테이트
  const [regCheckPw, setRegCheckPw] = useState('');  // 비번 , 중복체크 비번 일치 여부

  const handleCheckPw = (e)=>{  // 비번 일치 여부 확인 메서드
    setCheckPw(e.target.value);

    if(ejoinUser.userPassword === checkPw){
      setRegCheckPw(true);
    }else{
      setRegCheckPw(false);
    }
            ...
            <div className='ejoin-box'>
              ...
              <div className='ejoin-form'>
                ...
                <div className='ejoin-form-inner type2'>
                  ...
                  {isRegexs.RPassword === false ? <p className="join_input_regexTxt">*영문 혹은 영문+숫자 조합 6자 이상 입력해주세요</p> : null}
                </div>
                ...
                <div className='ejoin-form-inner type2'>
                  ...
                  {regCheckPw === false ? <p className="join_input_regexTxt">*비밀번호가 일치하지 않습니다</p> : null}  {/*😡😡😡이거 이상함 😡😡😡*/}
                </div>
